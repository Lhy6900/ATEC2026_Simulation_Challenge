...
DEFAULT_BLIND_STABILIZE_STEPS = 2
DEFAULT_STABILIZE_VEL_GAIN = 0.04
...
DEFAULT_INITIAL_LAST_ACTION = "zero"
...
# Counter-650 TaskD handoff pose in the CrossPitBox training frame.  The official
# TaskD world frame is shifted by this amount relative to the training env-local
# frame captured from the GUI-equivalent 13 s state.
CROSS_PIT_ENV_ORIGIN_X = -4.199999809265137
CROSS_PIT_HANDOFF_LOCAL_X = 2.4567047357559204
CROSS_PIT_HANDOFF_LOCAL_Y = -0.9189060926437378
CROSS_PIT_HANDOFF_ROOT_Z = 0.733408510684967
CROSS_PIT_HANDOFF_YAW = 0.24330876500455584
...
# Raw JointPositionAction command corresponding to the CrossPitBox-v1 training
# reset joint pose when the official TaskD action term uses scale=0.5 and the
# robot default joint position as offset. This is useful for diagnostics and
# optional stabilization, but the trained actor already outputs raw
# JointPositionAction commands in this same space.
CROSS_PIT_RESET_ACTION = torch.tensor(
    [
        ...
        -0.2835733079910278,
        0.2907680571079254,
        -0.006896710395812966,
        -0.2589409053325653,
        0.006258697714656591,
        1.0615340805053712,
        -0.8532652044296265,
        0.08029133081436157,
        -0.6350610852241516,
        -0.271651029586792,
        0.6036292910575867,
        -0.021711778640747026,
        0.00040976643562318227,
        -0.00011282000195933506,
        0.010751614570617685,
        5.899920506635681e-05,
        0.005196995101869106,
        -0.00019784501637332141,
        -0.0077537417411803755,
        0.02194030642509459,
        0.009978578425943851,
        0.019359478950500497,
        6.130044494057074e-05,
        0.0062080686911940575,
        0.0012295583728700876,
        0.0001257028877735128,
        -0.0001305672824382792,
        -2.029868960380654e-05,
        7.723313570022483e-05,
    ],
    dtype=torch.float32,
)
...
class CrossPitBoxPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        self.initial_last_action = os.environ.get(
            ...
            DEFAULT_INITIAL_LAST_ACTION,
        ).strip().lower()
        if self.initial_last_action not in ("zero", "env"):
            raise ValueError(
                "ATEC_CROSS_PIT_INITIAL_LAST_ACTION must be 'zero' or 'env', "
    ...
    def _validate_deploy_defaults_if_requested(self) -> None:
        if not self.require_deploy_defaults:
            return
        errors: list[str] = []
        if self.model_path.name != "cross_pit_box_v1_5_blind_model_19999.pt":
        ...
        if self.initial_last_action != "zero":
    ...
    def _resolve_model_path(self, explicit: str | Path | None) -> Path:
        value = (
            ...
            or "cross_pit_box_v1_5_blind_model_19999.pt"
        )
        ...
        if resolved_path is None:
            raise FileNotFoundError(
                ...
                "Place cross_pit_box_v1_5_blind_model_19999.pt in demo/ or set ATEC_CROSS_PIT_BOX_CHECKPOINT."
            )
