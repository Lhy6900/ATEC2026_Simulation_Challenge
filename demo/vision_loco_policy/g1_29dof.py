...
from g1_locomotion.tasks.manager_based.g1_locomotion import mdp
# import isaaclab.terrains as terrain_gen
import g1_locomotion.tasks.manager_based.g1_locomotion.terrains as terrain_gen

##
# Pre-defined configs
##
from g1_locomotion.tasks.manager_based.g1_locomotion.terrains.terrain_cfg import ROUGH_TERRAINS_CFG  # isort: skip
from g1_locomotion.tasks.manager_based.g1_locomotion.terrains.finetune_terrain_cfg import FINETUNE_ROUGH_TERRAINS_CFG
from g1_locomotion.tasks.manager_based.g1_locomotion.assets.robots.unitree import UNITREE_G1_29DOF_CFG as ROBOT_CFG
##
# Scene definition
##
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    height_scanner = RayCasterCfg(
        ...
        ray_alignment="yaw",
        pattern_cfg=patterns.GridPatternCfg(resolution=0.05, size=[1.6, 1.0]),  # 分辨率0.05m，范围1.6mx1.0m，尺寸33x21
        # pattern_cfg=patterns.GridPatternCfg(resolution=0.1, size=[1.6, 1.0]),   # 分辨率0.1m，范围1.6mx1.0m，尺寸17x11
        debug_vis=False,
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        def __post_init__(self):
            self.enable_corruption = True   # 加噪（默认Fasle）
            self.concatenate_terms = True   # 拼接（默认True）
...
class RewardsCfg:
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "threshold": 1,
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["(?!.*ankle.*).*"]), # 除了脚之外的所有部位
        },
    )
    ...
    # Encourage feet to land on locally flat patches (uses contact positions + raycast sampling)
    flat_feet_contact = RewTerm(
        ...
        params={
            ...
            "patch_radius": 0.05,
            "num_ring_samples": 8,
            "max_height_variation": 0.02,
            "reward_when_flat": 1.0,
    ...
    #     weight=0.05,
    #     params={
    #         "sensor_cfg": SceneEntityCfg("feet_contact", body_names=".*_ankle_roll_link"),
    #         "patch_radius": 0.05,
    #         "num_ring_samples": 8,
    #         "max_height_variation": 0.02,
    #         "reward_when_flat": 1.0,
    ...
    base_contact = DoneTerm(
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_forces", 
                body_names=["torso_link", ".*_shoulder_.*_link", "pelvis"]
...
class G1RoughEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.rewards.flat_feet_contact.weight = 0.0
        ...
        # self.rewards.flat_feet_contact.weight = 0.2    #
    ...
    def __post_init__(self):
        ...
        if self.scene.terrain.terrain_generator is not None:
            ...
            self.scene.terrain.terrain_generator.sub_terrains =  {
                ...
                #     proportion=0.5, stake_height_max=0.0, stake_side_range=(0.20, 0.20), stake_gap_range=(0.3, 0.3),
                #     column_gap_range=(0.1, 0.1), column_jitter=0.00, holes_depth=-2.0, platform_width=2.0, border_width=0.25
                # ),
                "stakes2": terrain_gen.HfAlternateColumnStakesTerrainCfg(
                    proportion=0.5, stake_height_max=0.0, stake_side_range=(0.2, 0.2), stake_gap_range=(0.1, 0.1),
                    column_gap_range=(0.2, 0.2), column_jitter=0.0, holes_depth=-2.0, platform_width=2.0, border_width=0.25
                ),
                ...
                #     proportion=0.1, rail_height_range=(0.15, 0.15), rail_thickness_range=(0.2, 0.2), platform_width=2.0
                # ),
            }
