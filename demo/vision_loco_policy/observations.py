...
def elevation_map(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg, noise: bool = False) -> torch.Tensor:
    ...
    if noise:
        ...
        offset_noise = (torch.rand(N, 1, device=env.device) * 0.1 - 0.05)   # Uniform noise between -5cm and 5cm
        sensor_coords[..., 2] += offset_noise
        
    # Clip height values
    sensor_coords[..., 2] = torch.clamp(sensor_coords[..., 2], min=-1.2, max=0.0)

    # # --- Randomly shift the map selection window ---
    # # shift the window up/down and left/right
    # sensor_coords = sensor_coords.reshape(N, 23, 35, 3)
    # resolution = 0.05
    # if noise:
    #     # Vertical shift (up or down by one grid cell)
    #     # dx = torch.randint(-1, 2, (1,)).item() # -1, 0, or 1
    #     # dy = torch.randint(-1, 2, (1,)).item() # -1, 0, or 1
    #     dx = 0
    #     dy = 0
    #     col_start = 1 + dx
    #     col_end = 34 + dx
    #     row_start = 1 + dy
    #     row_end = 22 + dy
    #     # Adjust coordinates for the shift
    #     sensor_coords[..., 0] -= dx * resolution
    #     sensor_coords[..., 1] -= dy * resolution
    # else:
    #     # Default: no vertical shift
    #     col_start, col_end = 1, 34
    #     row_start, row_end = 1, 22 
    # # Apply the calculated crop
    # sensor_coords = sensor_coords[:, row_start:row_end, col_start:col_end, :]
    # # print("elevation_map:", sensor_coords.reshape(N, 21*33*3)[0, :200])
    # return sensor_coords.reshape(N, 21*33*3)
