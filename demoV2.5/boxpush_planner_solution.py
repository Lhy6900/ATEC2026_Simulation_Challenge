    ...
    def __init__(
        ...
        *,
        ...
        self.planner_checkpoint = self._resolve_path(
            ...
            default_name="boxpush_planner_v2.pt",
            fallback_names=("boxpush_planner_10hz.pt", "boxpush_planner.pt"),
        )
    ...
    def _resolve_path(
        ...
        *,
        ...
        if explicit is not None:
            candidate_values.append(explicit)
        env_value = os.environ.get(env_name)
        if env_value is not None:
            candidate_values.append(env_value)
        candidate_values.append(default_name)
        candidate_values.extend(fallback_names)
        ...
        tried_text = ", ".join(str(path) for path in tried_paths)
        raise FileNotFoundError(
            f"Missing required policy asset. Tried: {tried_text}. "
